...
datos.append("acc",e.target.dataset.acc);inputs.forEach((valor)=>{datos.append(valor.name,valor.value)});try{const url=formulario.getAttribute("action");const response=await fetch(url,{headers:{'Token':mikey},method:'POST',body:datos});let datosRecibido=await response.json();console.log(typeof datosRecibido);if(datosRecibido["ok"]){mensaje(datosRecibido['mensaje'],"/tienda")}else{console.log(datosRecibido["ok"]);alerta("No podido actualizar los datos del producto: "+datosRecibido['error'])}}catch(error){console.log(error);alerta('Error al contactar con el servidor')}}
