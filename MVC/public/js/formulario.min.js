...
try{const url=e.target.getAttribute("action");const metodo=e.target.getAttribute("method")||'POST';const response=await fetch(url,{headers:{'token':mikey},method:'POST',body:datos_post});const datosStr=await response.clone().text();console.log("recibido:"+datosStr);const datosRecibidos=await response.json();if(datosRecibidos.ok){mensaje('todo ok: '+datosRecibidos.mensaje,'/contacto')}else{alerta(datosRecibidos.error)}}catch(error){console.log(error)}finally{ocultarLoader()}}