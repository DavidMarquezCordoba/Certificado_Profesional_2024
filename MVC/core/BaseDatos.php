...
class BaseDatos {
    ...
    function __construct() {
        ...
        $this->conexion = $this -> conectar(); // la instancia a la bbdd
    }

    // Función para conectar a una base de datos, las credenciales para la conexión están en un archivo llamado "db.php" dentro de la carpeta "config" -- esto lo cambiamos por las variables de entorno
    ...
    public function guardar($consultaSQL, $parametros = []) {
        $respuesta = $this->realizaConsulta($consultaSQL, $parametros);
        if($respuesta[0]){ // miro si ha sido correcta la insercion, y voy a pedirle la ultima id
    ...
    private function realizaConsulta($consultaSQL, $parametros){
        ...
        return [$resultadoConsulta, $consultaPreparada->get_result()]; // aqui devuelve true/false, y los resultados (si fuese un select, si fuese un insert, no devuelve nada)
    }
