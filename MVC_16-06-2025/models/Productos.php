...
class Productos {
    ...
    public function nuevoProducto($datosRecibidos) {
        $productoBuscado = $this->buscarPorCodigoBarras($datosRecibidos['newcod']);
        $productoBuscado = json_decode($productoBuscado,true);
        if(!isset($productoBuscado['error'])){  // Si existe un producto con este código de barras
            return ['ok' => false, 'error' => 'El producto Ya existe y es ' . $productoBuscado['nombre']];
        }  

        $datosRecibidos['foto'] = $this->gestionaFoto('/img/avatares/youngpeople.png');
